...
test("keeps independent table-info column search local to the table-info view", () => {
  const source = readFileSync("apps/desktop/src/components/grid/TableInfoView.vue", "utf8");

  assert.match(source, /const filteredColumns = computed/);
  assert.match(source, /column\.data_type/);
  assert.match(source, /column\.column_default/);
});
