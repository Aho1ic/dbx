...
test("table info tabs use table titles, labels, and tooltip metadata", () => {
  const restoreStorage = installMemoryStorage();
  setActivePinia(createPinia());
  useConnectionStore().addEphemeralConnection(conn("conn-1"));
  const t = (key: string) => {
    if (key === "grid.tableInfo") return "Table Info";
    return key;
  };
  ...
  try {
    ...
    assert.equal(tabDisplayTitle(tab, t), "users@app.audit");
    assert.equal(tabModeLabel(tab, t), "Table Info");
    assert.deepEqual(tabTooltipLines(tab, t).at(-1), { label: "tabs.tooltipTable", value: "users" });
  } finally {
    restoreStorage();
  }
});
