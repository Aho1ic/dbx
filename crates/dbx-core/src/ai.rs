...
mod tests {
    ...
    async fn transport_errors_do_not_expose_request_query_parameters() {
        // Keep the port occupied and accept the request before dropping the
        // socket. This guarantees a connection-level failure without racing
        // another local service that could reuse the released port and return
        ...
        let result = client.get(format!("http://127.0.0.1:{port}/models?key={secret}")).send().await;
