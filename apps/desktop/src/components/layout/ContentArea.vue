...
import type { QueryTab, ConnectionConfig, TreeNode, VectorCollectionMeta, ObjectBrowserViewport } from "@/types/database";
...
function openTableInfoTab() {
  const tab = props.activeTab;
  if (!tab.connectionId || !tab.tableMeta) return;
    ...
    <template v-else-if="activeTab.mode === 'data'">
      <div class="flex-1 min-h-0 flex flex-col">
        <div class="h-9 shrink-0 border-b bg-background/80 px-3 flex items-center gap-2 text-xs">
          ...
          <span class="ml-auto" />
          <Button v-if="activeTab.result && activeTab.tableMeta && activeTab.connectionId" variant="ghost" size="sm" class="h-5 text-xs px-1.5 shrink-0" @click="openTableInfoTab()"><TableProperties class="h-3.5 w-3.5" />{{ t("grid.tableInfo") }}</Button>
