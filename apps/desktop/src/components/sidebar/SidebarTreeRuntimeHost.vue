...
import type { ColumnInfo, DatabaseType, TableInfoTab, TreeNode, TreeNodeType } from "@/types/database";
...
async function openTableInfoTab(node: TreeNode = activeNode.value, initialTab: TableInfoTab = "columns") {
  if ((node.type !== "table" && node.type !== "view" && node.type !== "materialized_view") || !node.connectionId || !hasTreeNodeDatabaseContext(node)) return;
  try {
    await connectionStore.ensureConnected(node.connectionId);
    const config = connectionStore.getConfig(node.connectionId);
    const querySchema = connectionObjectTreeQuerySchema(config, node.database, node.schema || node.database);
    const columns = await api.getColumns(node.connectionId, node.database, querySchema, node.label, node.catalog);
...
function buildObjectSidebarMenu(context: SidebarMenuFactoryContext): boolean {
  ...
  if (node.type === "table" || node.type === "view" || node.type === "materialized_view") {
    ...
    items.push({ label: t("grid.tableInfo"), action: () => void openTableInfoTab(node), icon: TableProperties });
    if (node.type === "table") {
      items.push({
        label: t("contextMenu.viewDdl"),
        action: () => void openTableInfoTab(node, "ddl"),
        icon: FileCode,
      });
    }
