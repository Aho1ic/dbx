...
export const useQueryStore = defineStore("query", () => {
  ...
  const _persistSnapshot = computed(() =>
    tabs.value.map((t) => ({
      ...
      tableInfoTarget: t.tableInfoTarget,
      tableInfoActiveTab: t.tableInfoActiveTab,
  ...
  function openTableInfo(connectionId: string, database: string, schema: string | undefined, tableName: string, columns: NonNullable<QueryTab["tableInfoTarget"]>["columns"], primaryKeys: string[], options: { catalog?: string; initialTab?: TableInfoTab } = {}) {
    const existing = tabs.value.find(
      (tab) => tab.mode === "tableInfo" && tab.connectionId === connectionId && tab.database === database && (tab.tableInfoTarget?.catalog || "") === (options.catalog || "") && (tab.tableInfoTarget?.schema || "") === (schema || "") && tab.tableInfoTarget?.tableName === tableName,
    );
    if (existing) {
      existing.tableInfoTarget = { catalog: options.catalog, schema, tableName, columns, primaryKeys };
      if (options.initialTab) existing.tableInfoActiveTab = options.initialTab;
    ...
    const tab: QueryTab = {
      ...
      isExplaining: false,
      mode: "tableInfo",
      tableInfoTarget: { catalog: options.catalog, schema, tableName, columns, primaryKeys },
      tableInfoActiveTab: options.initialTab ?? "columns",
    };
    tabs.value.push(tab);
    activeTabId.value = id;
    return id;
  ...
  function duplicateTab(id: string) {
    ...
    const newTab: QueryTab = {
      ...
      tableInfoTarget: original.tableInfoTarget
        ? {
            ...original.tableInfoTarget,
            columns: original.tableInfoTarget.columns.map((column) => ({ ...column })),
            primaryKeys: [...original.tableInfoTarget.primaryKeys],
          }
        : undefined,
      tableInfoActiveTab: original.tableInfoActiveTab,
  ...
  function tabMatchesDroppedTableObject(tab: QueryTab, target: DroppedTableObjectTarget): boolean {
    ...
    if (tab.mode === "tableInfo") {
      const tableInfoTarget = tab.tableInfoTarget;
      if (!tableInfoTarget || tableInfoTarget.tableName !== target.name) return false;
      if ((tableInfoTarget.catalog || "") !== (target.catalog || "")) return false;
      return targetSchemas.has(normalizeOptionalSchema(tableInfoTarget.schema ?? tab.schema));
    }
  ...
  function closeDroppedTableObjectTabs(target: DroppedTableObjectTarget) {
    // A dropped table-like object makes existing data/structure/info tabs stale; close
