...
async function openTableTarget(target: NavigationTarget, options: { tableInfoTab?: TableInfoTab } = {}) {
  ...
  const openRequestedTableInfo = (columns: Awaited<ReturnType<typeof api.getColumns>>, primaryKeys: string[]) => {
    if (!options.tableInfoTab) return;
    queryStore.openTableInfo(target.connectionId, target.database, target.schema, target.tableName, columns, primaryKeys, {
      catalog: target.catalog,
      initialTab: options.tableInfoTab,
    });
  };
  ...
  try {
    ...
    if (config.db_type === "neo4j") {
      ...
      openRequestedTableInfo(columns, primaryKeys);
      return;
    }
    const sql = await buildTableSelectSql({
      databaseType: effectiveDbType,
